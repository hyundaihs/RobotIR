...
class GLProgram {
    ...
    fun drawFrame() {
        ...
        // bind textures
        GLES20.glActiveTexture(GL_TEXTURE0 + _textureI)
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, _ytid)
        GLES20.glUniform1i(_yhandle, _tIindex)

        GLES20.glActiveTexture(GL_TEXTURE0 + _textureII)
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, _utid)
        GLES20.glUniform1i(_uhandle, _tIIindex)

        GLES20.glActiveTexture(GL_TEXTURE0 + _textureIII)
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, _vtid)
     ...
     */
    private fun createBuffers(vert: FloatArray?, coord: FloatArray) {
        _vertice_buffer = ByteBuffer.allocateDirect(vert!!.size * 4)
        ...
        if (_coord_buffer == null) {
            _coord_buffer = ByteBuffer.allocateDirect(coord.size * 4)
